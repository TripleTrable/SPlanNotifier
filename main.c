...
int main(int argc, const char* argv[])
{
    ...
    uint8_t print = 0;
    uint8_t dateCell = 0;
    const char* tooltip_str = "tooltip\">";
    const char* leftPos_pat = "left:";
    const char* dateDate_pat = "data-date";
    uint32_t pos; // pos of ttevent
    DateEntry* datePosMap[7];
    int8_t dateIter = -1;

    memset(datePosMap,0,sizeof(datePosMap));
    ...
    while ((read = getline(&line, &len, stdin)) != -1) {
        decode_html_entities_utf8(line,NULL);

        if(strstr(line,"ttweekdaycell"))
        {
            dateCell++; 
            dateIter++;

            const char* posStart = strstr(line,leftPos_pat);
            posStart+=strlen(leftPos_pat);
            const char* posEnd = strstr(posStart,"px");
            uint8_t size = posEnd - posStart;
        ...
        else if(strstr(line,dateDate_pat) && dateCell)
        {
            dateCell++;
            const char* posStart = strstr(line,dateDate_pat);
            posStart+=strlen(dateDate_pat)+2;
            const char* posEnd = strstr(posStart,"\">");
            uint8_t size = posEnd - posStart;
            datePosMap[dateIter]->date = malloc(size+1);
            memcpy(datePosMap[dateIter]->date,posStart,size);
            datePosMap[dateIter]->date[size] = ';';
            posStart+=strlen(dateDate_pat)+2; 

        }
        else if(strstr(line,"/div") && dateCell)
        {
            dateCell--;
        }
        else if(strstr(line,"ttevent week"))
        {
            print++;
            const char* posStart = strstr(line,leftPos_pat);
            posStart+=strlen(leftPos_pat);
        ...
        {
            const char* res = str_replace(line,"<br/>",";");
            printf(strstr(res,">")+1);
            free(res);
